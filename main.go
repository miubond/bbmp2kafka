...
func main() {
	...
	// 注册 Peer 事件处理器
	eventHandler := NewBMPEventHandler(ssp, *kafkaTopic, newTokenBucket(10, time.Second))
	server.SetBMPEventCallback(eventHandler)
	log.Info("BMP Peer event handler registered - Peer Up/Down events will be sent to Kafka")
