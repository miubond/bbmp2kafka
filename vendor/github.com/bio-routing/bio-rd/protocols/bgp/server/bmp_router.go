...
// BMPEventCallback 全局回调函数接口
type BMPEventCallback interface {
	OnPeerUp(routerIP, localIP, peerIP bnet.IP, localAS, peerAS uint32)
	OnPeerDown(routerIP, localIP, peerIP bnet.IP, localAS, peerAS uint32, reason string)
}

// globalBMPEventCallback 全局事件回调
var globalBMPEventCallback BMPEventCallback

// SetBMPEventCallback 设置全局 BMP 事件回调
func SetBMPEventCallback(callback BMPEventCallback) {
	globalBMPEventCallback = callback
}
...
func (r *Router) processPeerDownNotification(msg *bmppkt.PeerDownNotification) {
	...
	// 调用全局回调（发送到 Kafka）
	if globalBMPEventCallback != nil {
		routerIP, _ := bnet.IPFromBytes(r.address.To16())
		localIP := bnet.IP{}  // 这里没有 local IP 信息，使用空值
		reason := fmt.Sprintf("Reason code: %d", msg.Reason)
		globalBMPEventCallback.OnPeerDown(routerIP, localIP, peerAddr, 0, msg.PerPeerHeader.PeerAS, reason)
	}
...
func (r *Router) processPeerUpNotification(msg *bmppkt.PeerUpNotification) error {
	...
	if globalBMPEventCallback != nil {
		routerIP, _ := bnet.IPFromBytes(r.address.To16())
		globalBMPEventCallback.OnPeerUp(routerIP, localAddress, peerAddress, 0, msg.PerPeerHeader.PeerAS)
	}
